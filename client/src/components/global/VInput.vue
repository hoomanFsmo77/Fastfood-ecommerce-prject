...
<template>
  <template v-if="type==='text'">
    <div class="input-wrapper">
      <label v-if="label" class="label" :for="id">{{label}}
        <template v-if="require">*</template></label>
      <input  :class="inputClass" :placeholder="placeholder" :type="type" :disabled="disable" :value="modelValue" @input="emit('update:modelValue',$event.target.value)" :id="id" class="input input-primary" />
    </div>
  </template>
